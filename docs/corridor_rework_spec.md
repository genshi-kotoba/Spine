...
- v2（2026-09-05，主代理，固定走廊迭代）：① 特异点地面判定区可视化（JudgeZone 半透明带 + 亮线 + 「屏息」Label）；② 遮罩改昏暗透光（DarknessMask/BreathSystem darkness_color alpha → 0.55 深蓝黑）；③ 屏息判定阈值 0.5s→1.0s：长按 1s 触发全屏昏暗（BreathDim 半透明 ColorRect）+ 角色身边粒子震动 + 屏幕震动（Corridor 动态创建；呼吸缺氧不再由长按触发）；④ 未屏息回传增加红色粒子震撼 + 强震动。影响：Corridor.gd/BreathSystem.gd/DarknessMask.gd/SpecialPointLayer.gd/CorridorAssembly.gd；回滚要点：git revert 本提交。
