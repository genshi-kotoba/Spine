...
func run_self_check() -> bool:
	...
	var rocking_forwarded: bool = _breath_shake != null and _breath_shake.has_method("is_rocking") \
		and _breath_shake.is_rocking() and not is_zero_approx(_breath_shake.get_rocking_target())
	var pulse_ok: bool = _breath_dim_rect != null and _breath_dim_rect.visible and _breath_dim_rect.color.a > 0.0 and _breath_dim_rect.color.a < 1.0 and rocking_forwarded
