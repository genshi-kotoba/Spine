...
- **卧室门（用户定案，原"闭锁门"）**：**正视房门样式**（门框 110×260 + 门扇 + 门把 + 红色锁标）、**默认锁着（锁标可见）**、**纯背景不物理阻挡**、玩家进入检测区触发门扇「正着开向镜头」（缩放放大+淡出，不倾斜）、离开后延迟关闭；GameState 仍存 locked（解锁玩法待用户设计，interact() 空实现）；实例位置客厅中央 (1920,858)。
