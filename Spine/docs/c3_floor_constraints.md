...
1. 资产全占位符（灰盒/色块白模）。
2. 输出分辨率 1920×1080，游戏画幅 2.35:1（内部视口 1920×817），窗口保留上下黑边。
...
- 输出窗口：1920×1080（固定窗口，见 §7）；游戏内部视口 1920×817（2.35:1），由 stretch/aspect=keep 在窗口内垂直居中，约上 131px、下 132px 黑边。
- 三房均分 2.35:1 游戏区：书房 x∈[0,1280]、客厅 x∈[1280,2560]、餐厅 x∈[2560,3840]；垂直内容范围沿用内部视口 y∈[211,1028]。
- 分隔墙 x=1280 与 x=2560；每堵墙一个门洞，木门置于门洞（通行高度，实现自由）。
- 地板 y=988（游戏区站立面）；顶面带 y∈[211,251]。
- 出生点（规格⑥）：书房内，建议 (320, 地板站立高度)。
- 闭锁卧室门（规格⑦）：客厅中央区域，建议 x≈960、落地于地板 y=1160；位置以「客厅中央」为准，精确坐标可微调。

### 4.2b 画幅 2.35:1 + 房间宽度 2/3 画幅（2026-09-05 用户定案 · 覆盖本节与 §4.3 的数值）

- 游戏画幅 16:9 → **2.35:1**：取景宽 1920 不变，游戏区高 **817**；固定输出窗口 1920×1080；上下黑边约**上 131px / 下 132px**（无场景内容）。
- **每个房间宽 = 2/3 画幅（1280px）**：书房 x∈[0,1280]、客厅 x∈[1280,2560]、餐厅 x∈[2560,3840]；地图总宽 3840；分隔墙 x=1280 / x=2560；侧墙 x=0 / x=3840。
- 相机：地图变宽后横向卷动（map_min_x=0、map_max_x=3840，clamp 自动 [960,2880]）；竖直固定取景中心 y=619.5（camera_position_offset=(0,-336.5)，相机语义：中心=玩家位置+offset）。
- 地板带：y∈[988,1028]（厚 40，站立面 988，宽 3840）；顶面带：y∈[211,251]（厚 40，宽 3840）。
- 分隔墙门洞：开口 y∈[708,1028]（高 320）；门楣柱 y∈[211,708]（高 497，中心 459.5）。
- **隔间自动木门（用户定案修订：等大小同一扇门）**：同一扇侧视门板 20×280 做开合——玩家触发通过时门板**水平收边**（scale.x→0.06，如摆入墙中）展示开门，离开后归位；**无正视门、无虚影、尺寸恒定**；碰撞语义不变（闭门阻挡、开门放行）；实例位置 (1280,868)、(2560,868)。
- **卧室门（用户定案修订：两种门属性区分）**：**静态锁门**——正视房门样式（门框 160×320 + 门扇 + 门把 + 红色锁标）、**默认锁着（锁标可见）**、**纯背景不物理阻挡**、**无检测区、不触发任何开门动画**；GameState 存 locked（解锁玩法待用户设计，interact() 空实现）；实例位置客厅中央 (1920,828)（底部贴站立面 988）。
- 隔间门正视开门态尺寸（用户定案：大一圈）：门框 160×320，恰好填满门洞开口 [708,1028]；门扇「正着开向镜头」（缩放放大+淡出，不倾斜）。
- 出生点：书房内 (320,948)（碰底 980，距站立面 988 留 8px 空隙，沿用 F5 教训）；角色站立后 y≈956。
- 景深：anchor_point=(1920,619.5)；三层 rect 宽 4400（覆盖全图+偏移余量）、半高 390/375/385；层中心 (1920,599.2/592/582)。
- 不变项/禁区：project.godot 不动；输入集不变；无新增玩法。

### 4.3 相机行为预期（横板前提）

- LevelScene 相机 clamp：`map_min_x=0`、`map_max_x=1920`。本关地图宽 = 视口宽（1920），因此相机水平位移恒为 0（画面固定居中 x=960）。**这是预期行为，不是 bug**。
- 本关相机为全帧固定取景：镜头中心 (960, 619.5)，内部视口 1920×817 入镜；输出窗口上下黑边由引擎留出。竖直方向不随角色移动，由实例固定（实现方式自由：实例配置偏移或模板导出）。
...
| `display/window/size/viewport_height` | 817 | 2.35:1 游戏内部视口高 |
| `display/window/size/window_width_override` | 1920 | 固定输出窗口 1920×1080 |
| `display/window/size/window_height_override` | 1080 | 固定输出窗口 1920×1080 |
...
- 固定输出窗口 1920×1080、不可缩放。
- 游戏内部视口 1920×817 垂直居中；上下约 131/132px 纯黑边。
...
| 对象 | 参数 | 建议默认 | 说明 |
|---|---|---|---|
| Player.gd（修改，仅加参数） | `move_speed` | 400（用户定案：×2 提速） | 已有 |
...
| ② | 2.35:1 黑边 | 输出窗口固定 1920×1080 且不可缩放；内部游戏视口 1920×817 垂直居中；上下约 131/132px 纯黑预留区；project.godot 设置符合 §7 | 1920×1080 运行截图 + project.godot 检查 |
| ③ | 模板化复用 | 模板场景/脚本无 C3 字面量；房间配置数据驱动；复制改配置可做下一层 | Select-String 扫描 + 结构走查 |
| ④ | 三房均分 | 书房/客厅/餐厅各 1280px 宽；分隔墙 x=1280、x=2560；墙体有碰撞 | 全景截图 + 场景树检查 |
...
# 3 窗口运行 + 黑边验证（固定 1920×1080：内部 1920×817 游戏视口、上下约 131/132px 纯黑预留区、窗口不可缩放）
F:\Godot\godot\godot.exe --path F:\Godot\Spine\Spine res://scenes/c3_floor.tscn
...
- A3 黑边定案（替代原「窗口缩放副产品」方案）：**固定窗口化、比例固定**；输出窗口 1920×1080；内部游戏视口 1920×817（2.35:1）垂直居中；上下约 131/132px 黑边；黑边为窗口内**预留文案呈现区**——本模块不实现文案内容，只预留留空。
