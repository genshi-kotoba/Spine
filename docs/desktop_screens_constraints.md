...
- 2026-09-06 v1.6（bug 修正）：v1.5 只改了场景 sub_resource，**运行时被 states 表覆盖**——MailIcon.gd / WorkIcon.gd 的 `_ready` 在 `super._ready()` 前填充 states（含 size 256×256 / 64×64），InteractableObject._ready → change_state → apply_state 把 shape.size 改回 states 值。后果：mail 运行时实为 256×256 @ (600,300)（y 172–428），整体吞掉 work 点击区，两 Area2D 重叠争抢 → 点 work 可能开 mailbox。修正：states 表 size 改为与场景一致（mail 50×50、work 50×70），场景偏移不变。教训：**InteractableObject 子类的 states 表 size 是运行时权威，场景 sub_resource 只是初值，两者必须同步**。v1.6 补充：对齐后运行时实测（headless 实例化打印 global rect）MailIcon x(575,625) y(275,325)、WorkIcon x(575,625) y(330,400)，与 v1.5 规格逐像素一致。
- 2026-09-06 v1.7：链接序列黑屏文字放大一倍（用户指示）。事实更正：**F9 的 32/24 字号从未接线**——场景与脚本均无字号覆盖，LinkTitle/LinkBody 一直按默认 16px 渲染。故「扩大一倍」以实际渲染为基准：两个 RichTextLabel 字号 16→32（LinkTitle 的 normal/bold/italics/bold_italics 四个 theme_override 全设，[b][i] 走 bold_italics_font），两行间距 20→40px（LinkBody offset_top 640→660、offset_bottom 700→720，LinkTitle 位置不动）。F9 废止，以本条为准。
