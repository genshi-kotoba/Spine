...
- **隔间自动木门（用户定案）**：闭门态=侧视门板 20×280；玩家触发通过时动画从侧视收成窄边并淡入**正视开门态**（门框 110×260 + 摆开门扇）；碰撞语义不变（闭门阻挡、开门放行）；实例位置 (1280,868)、(2560,868)。
- **卧室门（用户定案，原"闭锁门"）**：改为**正视房门样式**（门框 110×260 + 门扇 + 门把 + 红色锁标）、**纯背景不物理阻挡**、玩家进入检测区触发门扇摆开（−95°）、离开后延迟关闭；GameState 仍存 locked（解锁玩法待用户设计，interact() 空实现）；实例位置客厅中央 (1920,858)。
