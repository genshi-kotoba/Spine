...
func _char_advance(ch: String) -> float:
	var w: float = _font.get_string_size(ch, HORIZONTAL_ALIGNMENT_LEFT, -1, FONT_SIZE).x
	if w <= 0.0:
		w = float(FONT_SIZE)
	return w
...
func _show_line(line_idx: int) -> void:
	...
	var base_pos: Vector2 = _quadrant_center(quadrant)
	...
	while _jitter_running:
		for i: int in _char_labels.size():
			if is_instance_valid(_char_labels[i]):
				var offset: Vector2 = Vector2(randf_range(-JITTER_PX, JITTER_PX), randf_range(-JITTER_PX, JITTER_PX))
				_char_labels[i].position = _base_positions[i] + offset
